...
const Avatar = ({ name }: { name: string }) => {
  ...
  return (
    <div className="w-6 h-6 text-xs rounded-full bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 flex items-center justify-center font-medium border border-transparent shadow-sm">
      {initials}
    </div>
  );
};
...
const SidebarItem = ({ icon: Icon, label, active, onClick, hasSubmenu, expanded, onToggle, count, action }: any) => (
  <div 
    ...
    className={`group flex items-center justify-between px-3 py-1.5 text-sm rounded-md cursor-pointer transition-colors mb-0.5 select-none
      ${active 
        ? 'bg-gray-200/60 dark:bg-gray-700/50 text-gray-900 dark:text-gray-100 font-medium' 
        : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
  >
    <div className="flex items-center gap-2 overflow-hidden">
      {hasSubmenu && (
        <span 
          className="p-0.5 hover:bg-gray-300 dark:hover:bg-gray-600 rounded text-gray-400 dark:text-gray-500"
      ...
      <Icon 
        size={16} 
        className={`flex-shrink-0 ${active ? 'text-gray-800 dark:text-gray-200' : 'text-gray-500 dark:text-gray-500'}`} 
      />
    ...
    <div className="flex items-center gap-2">
      ...
      {action}
      <MoreHorizontal size={14} className="opacity-0 group-hover:opacity-100 text-gray-400 dark:text-gray-500 hover:text-gray-600" />
    </div>
  </div>
);
...
export default function Sidebar({
  sidebarOpen, setSearchQuery, searchQuery, activeRoute, setActiveRoute,
  projects, expandedMenus, toggleMenu, setViewMode, viewMode, onAddProject
}: SidebarProps) {
  return (
    <div className={`${sidebarOpen ? 'w-64' : 'w-0'} bg-gray-50/50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col transition-all duration-300 ease-in-out overflow-hidden relative flex-shrink-0`}>
      <div className="p-4 flex items-center gap-3 mb-2">
         <div className="w-6 h-6 bg-gray-800 dark:bg-white rounded-md flex items-center justify-center text-white dark:text-gray-900 font-bold text-xs shadow-sm">P</div>
         <div className="font-semibold text-gray-800 dark:text-gray-100 text-sm tracking-tight flex-1">ProjectFlow</div>
      ...
      <div className="px-3 mb-4">
         <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 px-2 py-1.5 rounded-md flex items-center gap-2 text-sm shadow-sm focus-within:ring-2 focus-within:ring-gray-200 dark:focus-within:ring-gray-700 transition-shadow">
            <Search size={14} />
            <input 
              type="text" 
              placeholder="Search..." 
      ...
      <div className="flex-1 overflow-y-auto px-2 space-y-6 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-800">
         <div>
              ...
              <div className="space-y-0.5">
                {projects.filter(p => p.type === 'project').map(p => (
                  <div key={p.id}>
                    ...
                    />
                    {activeRoute.id === p.id && (
                       <div className="ml-4 border-l border-gray-200 dark:border-gray-800 pl-2 space-y-0.5 mb-1">
                         <div onClick={() => setViewMode('board')} className={`text-sm py-1 px-2 rounded cursor-pointer flex items-center gap-2 ${viewMode === 'board' ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><Kanban size={14} /> Board</div>
                         <div onClick={() => setViewMode('list')} className={`text-sm py-1 px-2 rounded cursor-pointer flex items-center gap-2 ${viewMode === 'list' ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><ListIcon size={14} /> List</div>
                         <div onClick={() => setViewMode('gantt')} className={`text-sm py-1 px-2 rounded cursor-pointer flex items-center gap-2 ${viewMode === 'gantt' ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><BarChart2 size={14} /> Gantt</div>
                      </div>
                    )}
                  </div>
                ))}
                
                {projects.filter(p => p.type === 'doc').map(p => (
